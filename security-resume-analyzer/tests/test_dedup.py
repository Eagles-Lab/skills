...
from security_resume_analyzer.dedup import (
    ResumeSource,
...
def person(
    ...
    email: str | None = None,
    school: str = "示例大学",
    description: str = "使用 Python 编写安全工具",
) -> dict[str, object]:
    return {
        "basic_info": {
            "name": name,
...
def test_identity_plus_similarity_fallback_without_contacts() -> None:
    ...
    right = Resume.model_validate(person(description="使用 Python 编写安全工具并测试"))
...
def test_description_conflict_keeps_primary_and_records_conflict() -> None:
    rich = person(
        email="same@example.test", description="负责使用 Python 编写安全工具并完成单元测试"
    )
...
def test_secondary_unique_experience_is_added_once() -> None:
    ...
    second["security_activities"] = [
        {
            "organization": "实验室",
            "name": "检测规则",
            "duration": "2026",
            "description": "实现 Sigma 检测规则",
        }
    ]
    merged, _ = merge_candidates([source("a", first), source("b", second)])
    assert len(merged[0].resume.security_activities) == 1
