...
from security_resume_analyzer.errors import PDFExtractionError
from security_resume_analyzer.pdf import (
    PDFLimits,
    _check_deadline,
    extract_raw_pdf,
...
def test_deadline_empty_document_char_and_cell_limits(tmp_path, monkeypatch):
    monkeypatch.setattr("security_resume_analyzer.pdf.time.monotonic", lambda: 100.0)
    with pytest.raises(PDFExtractionError, match="exceeded"):
