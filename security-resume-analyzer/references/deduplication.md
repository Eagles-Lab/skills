...
plus duration. Project and security-activity identity require name plus either
organization or duration. For weaker partial keys, collapse exact duplicates
only. Once a key qualifies as strong, compare its complete normalized
`(organization/company, name, duration)` tuple. For conflicting descriptions,
retain the primary and record
`kept_primary`. Never concatenate facts or double-score evidence.
