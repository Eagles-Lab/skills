...
- each non-`other` project category;
...
duration; a strong project requires name plus organization, or name plus
duration when organization is absent. Compare eligible records by the complete
normalized organization/name/duration tuple. When a record lacks that strong
key, reject only an exact-record duplicate. Reject duplicate skill, technology,
or achievement values with the same grounding compaction: NFKC/case, fixed
aliases, and non-semantic whitespace/punctuation while preserving numeric
syntax. Emit `canonical_duplicate_list_item`. These checks prevent one raw
occurrence from
...
record scope before checking direct leaves. Require organization or name;
duration is only a tie-breaker and never an anchor by itself. A scope may span
multiple lines but ends at the next
explicit peer header or section heading. Every direct leaf must be grounded
...
gate, not permission to infer or paraphrase omitted facts.
...
semantic similarity, reordered paraphrases, or numeric approximation. Ground
project categories with controlled same-record source signals rather than the
enum spelling alone. For an unambiguous proof, the category signal and the
canonical organization/name anchor must occur on the same unique raw line. If
that anchor is missing, duplicated, or split across lines, map the category to
`other` instead of asserting a controlled enum. A locally negated signal such
as `不是课程设计`, including a negation omitted from canonical wording, never
