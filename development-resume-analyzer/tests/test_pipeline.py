...
from development_resume_analyzer.analyzer import (
    DevelopmentResumeAnalyzer,
    collect_data_quality_warnings,
    load_resume,
)
from development_resume_analyzer.batch import BatchProcessor
from development_resume_analyzer.cli import analyze_main, batch_main
from development_resume_analyzer.errors import AnalyzerError, InputValidationError
from development_resume_analyzer.models import Resume
from development_resume_analyzer.rendering import ReportRenderer
...
def test_missing_warnings_are_json_only(tmp_path: Path) -> None:
    output = tmp_path / "run"
    analyzer = DevelopmentResumeAnalyzer(output, clock=lambda: FIXED)
...
def test_contact_only_with_explicit_flag(tmp_path: Path) -> None:
    output = tmp_path / "run"
    DevelopmentResumeAnalyzer(output).analyze(FIXTURES / "complete.json", include_contact=True)
...
def test_prompt_injection_does_not_change_workflow_or_leak(tmp_path: Path) -> None:
    ...
    output = tmp_path / "run"
    DevelopmentResumeAnalyzer(output).analyze(canonical)
...
def test_ten_questions_and_seed_are_deterministic(tmp_path: Path) -> None:
    ...
    a = DevelopmentResumeAnalyzer(first, clock=lambda: FIXED)
    b = DevelopmentResumeAnalyzer(second, clock=lambda: FIXED)
...
def test_batch_deduplicates_cross_format_canonical_sources(tmp_path: Path) -> None:
    ...
    assert summary["scoring_profile"] == "cn-campus-software-development-general"
...
def test_parallel_results_are_consistent(tmp_path: Path, workers: int) -> None:
    ...
    for index in range(4):
        payload = {
            ...
            "projects": [{"description": "使用 Python 编写后端工具"}],
        }
...
def test_internal_artifact_builder_rejects_non_sha256_source_hash(tmp_path: Path) -> None:
    analyzer = DevelopmentResumeAnalyzer(tmp_path / "run")
    with pytest.raises(InputValidationError):
...
def test_template_failure_leaves_no_visible_output(tmp_path: Path) -> None:
    ...
    analyzer = DevelopmentResumeAnalyzer(output, renderer=renderer)
