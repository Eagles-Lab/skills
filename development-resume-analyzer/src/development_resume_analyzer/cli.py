...
from .analyzer import DevelopmentResumeAnalyzer
...
def analyze_main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="analyze-development-resume",
        description="Analyze one canonical development resume v1 JSON document.",
    )
    ...
    try:
        DevelopmentResumeAnalyzer(args.output_dir).analyze(
            args.extracted,
...
def batch_main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="batch-analyze-development",
        description="Deduplicate and analyze canonical development resumes.",
    )
...
def extract_main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="extract-development-resume-text",
...
def calibrate_main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="calibrate-development-scoring",
        description="Evaluate private two-reviewer development scoring calibration.",
    )
