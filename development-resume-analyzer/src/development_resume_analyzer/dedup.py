...
class ResumeSource:
    ...
    sha256: str
    resume: Resume
    source_mapping_audit: dict[str, Any] | None = None
...
def _merge_group(group: list[ResumeSource]) -> MergedCandidate:
    ...
    for secondary in ranked[1:]:
        ...
        for collection in ("internships", "projects"):
    ...
    audits = tuple(
        item.source_mapping_audit
        for item in sorted(group, key=lambda value: value.sha256)
        if item.source_mapping_audit is not None
    )
    return MergedCandidate(
        resume, source_hashes, primary.sha256, output_name, tuple(conflicts), audits
    )
