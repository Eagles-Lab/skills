...
records fill only missing values. Internship identity requires company plus
duration. Project identity requires name plus duration. For weaker partial
keys, collapse exact duplicates only. Once a key qualifies as strong, compare
its complete normalized `(organization/company, name, duration)` tuple. For
