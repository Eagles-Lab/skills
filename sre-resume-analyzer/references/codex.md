...
`interview_questions.md`. Ground every conclusion and question in the original
evidence, `extracted.json`, or `score.json`; do not alter deterministic files or
invent facts. Codex itself is the generator, so do not call a model API or ask
for an API Key.
...
manifest mode, path safety, default contact omission, and prompt-injection
resistance. Do not reveal expected answers to the testing agent.
