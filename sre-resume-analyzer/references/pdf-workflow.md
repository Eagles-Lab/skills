...
6. The fallback output is named `raw_extraction.json`. Never pass it directly
   to `analyze-resume` or rename it `extracted.json`.
...
10. Validate canonical JSON, then use the same Python analysis flow as every
    other format.
11. Delete raw extraction and temporary canonical staging after final output is
    validated.
