"""SRE canonical-fact adapter for the shared source-mapping audit core."""
...
def _claims(resume: Resume, raw_text: str) -> tuple[list[FactClaim], list[AuditViolation]]:
    ...
    internship_scopes = record_collection_scopes(
        raw_text,
        tuple(
            tuple(value for value in (record.company, record.duration) if value)
            if record.company
            else ()
            for record in resume.internships
        ),
    ...
    for index, (internship, raw_scope) in enumerate(
        zip(resume.internships, internship_scopes.scopes, strict=True)
    ):
        prefix = f"/internships/{index}"
        for field in ("company", "role", "duration", "description"):
            _claim(
                claims,
                f"{prefix}/{field}",
                getattr(internship, field),
                raw_scope_text=raw_scope,
            )
        for field in ("tech_stack", "achievements"):
            for item_index, value in enumerate(getattr(internship, field)):
                _claim(
                    claims,
    ...
    project_scopes = record_collection_scopes(
        raw_text,
        tuple(
            tuple(value for value in (record.name, record.duration) if value) if record.name else ()
            for record in resume.projects
        ),
    ...
    for index, (project, raw_scope) in enumerate(
        zip(resume.projects, project_scopes.scopes, strict=True)
    ):
        ...
        for field in ("tech_stack", "achievements"):
            for item_index, value in enumerate(getattr(project, field)):
                _claim(
                    claims,
                    f"{prefix}/{field}/{item_index}",
                    value,
...
def _section_violations(resume: Resume, raw_text: str) -> list[AuditViolation]:
    ...
    has_skills = any(resume.skills.model_dump(mode="python").values())
    if states["skills"] == "populated" and not has_skills:
...
def audit_source_mapping(raw_extraction_path: Path, resume: Resume) -> SourceMappingAuditResult:
    """Prove every populated SRE canonical fact is grounded in the raw resume."""
